/*
* @Author: devyojan
* @Date:   2018-05-23 01:07:30
* @Last Modified by:   devyojan
* @Last Modified time: 2018-05-23 03:28:43
*/
  ...
  },
  {
	...
	for (var i=0; i<response.length; i++) {
		...
		if (response[i].status != 404) {
			...
			if (response[i].stream != null) {
		
				$('.streamers').prepend("<a href='"+response[i].stream.url+"' target='_blank'>"+
					"<img src='"+response[i].stream.logo+"'>"+
					"<div class='stream'><h2>"+response[i].stream.display_name+
					"</h2><p>"+response[i].stream.status+"</p></div>");
			} else {
		
				$('.streamers').append("<a href='https://www.twitch.tv/"+response[i].display_name
					+"' target='_blank'>"+
					"<div class='stream'><h2>"+response[i].display_name+
					"</h2><p class='offline'>Offline</p></div>");
			}
		}
	}
